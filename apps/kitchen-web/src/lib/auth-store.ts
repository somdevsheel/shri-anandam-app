...
/** Same in-memory-token / httpOnly-refresh-cookie architecture as apps/admin-web (ADR-020) — see that app's auth-store.ts for the full rationale. */
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  hydrate: async () => {
    try {
      ...
      set({ isHydrated: true, isAuthenticated: true, accessToken: json.data.accessToken, staff: staffFromToken(json.data.accessToken) });
    } catch {
      set({ isHydrated: true, isAuthenticated: false, accessToken: null, staff: null });
    }
  },
