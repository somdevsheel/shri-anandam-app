...
## ADR-020: Admin-web auth — in-memory access token + httpOnly refresh cookie via Next.js Route Handlers

**Decision:** `apps/admin-web` (Next.js, App Router) authenticates through three Route Handlers under `app/api/auth/*` (`login`, `refresh`, `logout`) that act as a thin BFF over `services/api`'s stateless Bearer-JWT staff auth — the same `POST /auth/staff/login` / `POST /auth/refresh` / `POST /auth/logout` endpoints the owner-mobile app uses. `login` and `refresh` write the refresh token into an `httpOnly`, `sameSite=lax`, path-scoped (`/api/auth`) cookie (`src/lib/server-config.ts`'s `refreshCookieOptions`) and return only the short-lived access token in the JSON body. The client (`src/lib/auth-store.ts`, a Zustand store) holds that access token **in memory only** — never `localStorage`/`sessionStorage` — and calls `hydrate()` (POST to `/api/auth/refresh`, a silent refresh against the cookie) once on app load. Every other admin-web request (`src/lib/api-client.ts`) goes straight from the browser to `services/api` with `Authorization: Bearer <token>`, not proxied through Next.js — CORS is already allow-listed per `docs/architecture/security-architecture.md`, and there's nothing in those requests a proxy hop would protect that the httpOnly cookie doesn't already protect by never reaching browser JS.

**Why:** `services/api`'s auth model has no first-class cookie support — it's stateless Bearer JWT designed around mobile clients holding tokens in `expo-secure-store` (OS keychain-backed, not available to a browser). A web admin panel that can trigger refunds, deactivate staff/customer accounts, and change role permissions cannot store a 30-day refresh token (`JWT_REFRESH_TTL`) anywhere a successful XSS could read it — `localStorage`/`sessionStorage`/a non-`httpOnly` cookie are all script-readable. An `httpOnly` cookie is the standard mitigation, and Next.js Route Handlers are the natural place to bridge "cookie-based browser session" to "Bearer-token API" without services/api itself needing to grow cookie support it has no other use for.

**Consequence:** there is no server-side way to check "is this request authenticated" (no middleware/`proxy.ts` route guard) — the httpOnly cookie's mere presence doesn't prove the refresh token inside it is still valid without the network round trip `hydrate()` already makes, and the access token that *would* prove it lives only in the browser's memory, invisible to the server. Route protection is therefore a client guard (`app/(dashboard)/layout.tsx`): block on `hydrate()`, redirect to `/login` if it comes back unauthenticated. A hard page reload always shows a brief loading state while the silent refresh resolves — the accepted trade-off for keeping the long-lived credential unreachable by any injected script.

**Verified live:** booted `services/api` and `admin-web` together and drove the actual cookie lifecycle with real HTTP requests (not just code review) — login returns an access token with no refresh token in the body and sets the `httpOnly` cookie (confirmed via a curl cookie jar); refresh using that cookie rotates it and returns a fresh access token; refresh with no cookie 401s; logout clears the cookie and invalidates the refresh token server-side, confirmed by a subsequent refresh attempt also 401ing.

## ADR-021: Admin customers endpoint and `customer.read`/`customer.update` permissions added for Phase 9

**Decision:** `services/api`'s only customer-facing routes were `customers/me/*` (a customer managing their own profile — no permission check, scoped to the caller's own id by construction). Phase 9's admin-web Customers screen needs a staff member to look up *any* customer, which is a different capability the RBAC model didn't have a permission for. Added `Permission.CUSTOMER_READ`/`CUSTOMER_UPDATE` to `packages/shared-types`' enum (granted to MANAGER by default, plus `CUSTOMER_READ` to SUPPORT — a support role plausibly needs to look up a customer without being able to deactivate one), and a new `AdminCustomersController` at `admin/customers` — a separate controller and the first `admin/`-prefixed path in this codebase, rather than adding a `:id` route onto `CustomersController` (whose base path `customers/me` is a literal exact match; mixing a parameterized route onto the same controller risks NestJS route-ordering ambiguity for no benefit).

**Why the customer's own order-detail relation needed a matching addition:** the owner-mobile app's "call customer" button (Phase 8) and admin-web's customer detail screen both need a customer's name/phone, but `ORDER_DETAIL_INCLUDE` in `orders.service.ts` had no `customer` relation — added a minimal `{mobileNumber, name}` select (not the full `Customer` record).

**Verified live:** registered a live device, created an order, and confirmed order detail now returns `customer: {mobileNumber, name}`; listed/searched/fetched/deactivated a real customer through `admin/customers`; confirmed a staff member without `customer.read` (a freshly created KITCHEN-role account) gets a 403 with the expected `FORBIDDEN`/`Missing required permission(s): customer.read` body, not a silent pass-through.

## ADR-022: Kitchen PWA — polling now, hand-rolled service worker, `sortOrder` bug fixed along the way

**Decision:** `apps/kitchen-web` (Next.js 16 App Router) reuses admin-web's exact auth architecture (ADR-020 — its own httpOnly-cookie BFF, its own cookie name so the two apps never share a session) but is otherwise a much narrower app: a single order-queue screen plus an order-detail screen, large-touch-target styling (56px min button height, 18px base font), and real PWA installability via Next's native `app/manifest.ts` plus a ~40-line hand-written `public/sw.js` — no `next-pwa`/Workbox dependency, since this app's actual need (installable + cache the build's own content-hashed static assets) doesn't warrant one, and this session hadn't verified such a plugin's compatibility with Next 16's Turbopack-first build. The service worker deliberately never caches `/api/auth/*` or any cross-origin request (order data goes straight to services/api, never through this app's own server) — a cached "still logged in" response after a real logout would be an actual bug, not a convenience.

**Realtime, honestly deferred:** `docs/architecture/production-architecture.md` names WebSockets as this app's eventual transport, but that's Phase 11's deliverable, not built yet. The queue polls every 5 seconds (shorter than admin-web's 15–20s, since this screen — unlike an admin dashboard — *is* the kitchen's primary signal that a new order exists) and a synthesized two-tone chime (`public/sounds/new-order.wav`, generated with Python's stdlib `wave` module — no external audio asset) plays when a new `PENDING` order id appears between polls. This is a real, working alert mechanism today, upgraded to a push-driven one when Phase 11 lands, not a placeholder waiting for it.

**A real bug found and fixed while building the queue, not after:** `OrdersService.listAdmin()`'s `sortBy`/`sortOrder` query params were accepted by `listOrdersAdminQuerySchema` but silently ignored — the query was always `orderBy: { createdAt: "desc" }`. A kitchen queue needs oldest-pending-first (so nothing silently ages at the bottom of a newest-first list), so this was fixed to `orderBy: { createdAt: query.sortOrder }` (`"desc"` is the schema's own default, so admin-web and owner-mobile — neither of which ever passes `sortOrder` — are unaffected). A second, more serious problem was caught before it shipped at all, not after: the queue's original design fetched `/orders` unfiltered with `sortOrder=asc`, which — once a restaurant has processed more than one page of orders ever — would return the OLDEST orders ever placed, not today's active ones. Fixed by querying each active status (`PENDING`/`ACCEPTED`/`PREPARING`/`READY`/`OUT_FOR_DELIVERY`) separately via `useQueries` and merging client-side; each status bucket is small by nature (a kitchen realistically never has 50+ simultaneous orders in one status), so this is both correct and cheap, unlike trying to page through an unfiltered, unscoped list.

**Verified live:** booted kitchen-web against the real API, logged in, placed a real order, and drove it through the full `PENDING → ACCEPTED → PREPARING → READY → DELIVERED` lifecycle via the exact status-transition calls the queue card's primary-action button and the detail screen's action list make — confirmed the `sortOrder=asc` fix actually returns oldest-first (and separately confirmed the unchanged `desc` default still holds for the no-`sortOrder` case), confirmed the manifest, icons, service worker, and alert sound are all served, and confirmed a KITCHEN-role permission set (`order.accept` only) correctly hides Reject/Cancel while a fuller permission set (as tested via the same owner login used throughout this session) sees all three.

## ADR-023: Realtime order updates — Socket.IO with a Redis adapter, in-process emission (not outbox-driven), permission-scoped rooms

**Decision:** `services/api` gained a `RealtimeModule`/`RealtimeGateway` (`@nestjs/websockets` + `socket.io`) alongside the REST API, authenticated the same way as REST (a JWT access token, passed via the socket.io handshake's `auth.token` rather than a header, since browser/RN WebSocket clients can't set arbitrary headers on the handshake the way `fetch` can) and re-verifying the subject is still active on every connection (mirroring `JwtStrategy.validate()`'s discipline). `OrdersService.createOrder()`/`transition()` call the gateway directly — in-process, not through the outbox — after their transaction commits, never from inside one (same "no variable-latency call inside a DB transaction" rule as ADR-017's Razorpay call and the outbox pattern itself, applied here even though a socket.io broadcast is in-process rather than a network call, because it can still block on backpressure). A `RedisIoAdapter` (`@socket.io/redis-adapter`, using the same `REDIS_URL` already a hard dependency per ADR-008) fans broadcasts out across every API replica, not just the one that handled the triggering request — required by this repo's own documented statelessness principle (`docs/architecture/production-architecture.md`, section 66: "API can run N replicas").

**Why in-process rather than outbox-driven (contrast with ADR-018's FCM pipeline):** the outbox pattern exists for at-least-once delivery to a recipient who might currently be offline (a phone with no connectivity, needing the push when it reconnects). A WebSocket client is, by definition, only ever reachable while actively connected to this exact API's socket.io layer — there is no "deliver later" case for it the way there is for FCM. A missed event because a client was transiently disconnected is corrected by that client's own reconnect and by every consuming screen's continued REST fetch/poll (kept everywhere as a fallback, at largely unchanged intervals) — the same "the database is authoritative regardless of push delivery" principle as the FCM pipeline, just applied to a strictly best-effort transport instead of an at-least-once one, matching what a WebSocket broadcast can actually promise.

**Room design, and a real bug this session's own live testing caught:** the first implementation joined each staff socket to `branch:{branchId}` rooms derived from their `BranchStaff` rows, mirroring the (reasonable-sounding) assumption that a staff member should only see their own branch's orders live. Live testing (a real socket.io client, not just code review) immediately surfaced that the seeded bootstrap OWNER account has zero `BranchStaff` rows — an owner isn't tied to one location — and so received no events at all under that design. Checking `OrdersService.listAdmin()` confirmed this wasn't a seed-data quirk to patch around: the REST endpoint applies no branch restriction beyond an optional client-supplied filter, so *any* staff member holding `order.read` can already read *any* branch's orders over REST today. The fix makes the WebSocket layer mirror that exactly — one shared `staff:orders` room, gated on `payload.permissions.includes(Permission.ORDER_READ)` (the same permission `OrdersController.list()` checks), not on branch assignment. Verified live three ways: a fully-permissioned staff socket receives `order:created` and `order:updated`; a customer socket receives `order:updated` only for their own orders (`customer:{customerId}` room); a staff socket for a role with no `order.read` (ACCOUNTANT) receives nothing at all, confirming the permission gate actually excludes rather than just failing to include by omission.

**Frontend wiring:** `apps/kitchen-web`'s queue (Phase 10) now plays its new-order sound and refreshes on the WebSocket event itself rather than the poll-diff it shipped with — the poll (unchanged interval) stays as the fallback. `apps/admin-web`'s dashboard layout opens one shared connection for every screen under it. `apps/customer-mobile` gained the connection on its order-history and order-detail screens.

**A prerequisite this phase actually needed, not scope creep:** `apps/customer-mobile`'s checkout screen was still Phase 5's explicit placeholder ("Checkout is on its way — Phase 6... not yet implemented") despite `services/api`'s orders module (Phase 6) being complete, tested, and already used by three other frontends. "Live status updates for customers" has nothing to attach to without a real checkout and a real order-tracking screen, so both were built here: a real `POST /orders` checkout flow (fulfillment type, address selection reusing the existing `AddressCard`/`useAddresses`, COD/Pay-at-Store — Razorpay's client-side checkout SDK was never integrated in this app and offering UPI/CARD/NET_BANKING here without it would be dishonest about what actually completes), a new "Orders" tab (history list), and an order detail/tracking screen with a Cancel action. All three — checkout, cancel, and the realtime status update itself — were verified against the real running API in this session, including watching a real customer socket receive `order:updated` the instant a staff member accepted the order over REST.

## ADR-024: Phase 12 hardening — Redis-backed rate limiting, a corrected metrics interceptor, and a response-envelope escape hatch

**Decision (rate limiting):** `ThrottlerModule` now uses `@nest-lab/throttler-storage-redis` (reusing the app's existing `RedisService`/`REDIS_CLIENT` connection, not a second one) instead of the package's default in-memory storage. The global default limit was also raised from 100/min to 300/min (`RATE_LIMIT_MAX_REQUESTS`).

**Why (Redis storage) — a real gap live-testing caught, not a speculative hardening pass:** stopping the Redis container mid-session (part of the same failure-testing pass documented in `docs/deployment/failure-testing.md`) and then calling `POST /auth/staff/login` succeeded completely, with **zero rate-limit enforcement** — the in-memory default doesn't care whether Redis exists at all. This directly contradicts `production-architecture.md`'s own Statelessness section, which states plainly that "rate-limit counters live in Redis, shared across instances." In-memory storage means two real problems, not one: (1) in the documented multi-replica deployment, each replica enforces its own separate counter, so the *effective* global limit is `configured limit × replica count`, not the configured number; (2) as observed live, the limiter can be silently defeated entirely by anything that affects only Redis, not the throttler's own (nonexistent) dependency on it. Fixed by wiring the Redis-backed storage adapter and re-verifying live: a `{<hash>:default}:hits` key now appears in Redis on the first throttled request, increments correctly, carries the configured TTL, and a `429` is correctly returned once the limit is exceeded.

**Why (raised default) — real math from this session's own apps, not a guess:** a Phase 12 load test (`docs/deployment/load-testing.md`) with an unthrottled flood correctly triggered mass `429`s as expected, but reviewing what *legitimate* traffic actually looks like exposed the old 100/min default as too tight: `apps/kitchen-web`'s queue screen alone issues 5 parallel per-status requests every 5 seconds — 60 requests/minute from a single open kitchen display, before counting `apps/admin-web`'s own polling, any other staff device on the same restaurant NAT'd connection, or customer traffic sharing that IP. Raised to 300/min to leave real headroom above that math (still far below what an actual abusive flood generates, as the same load test's 7,200 req/s run demonstrated) rather than the platform's own dashboards legitimately tripping its abuse protection.

**Decision (metrics correctness) — two bugs caught live before either shipped:**
1. The first version of Phase 12's HTTP metrics recorder was a `NestInterceptor`. Live testing showed it silently never recorded a single Guard-rejected request (401 from `JwtAuthGuard`, 403 from `PermissionsGuard`, 429 from `ThrottlerGuard`) — NestJS's request lifecycle is Middleware → Guards → Interceptors, so an interceptor never runs for anything a Guard already rejected. Rewritten as `MetricsMiddleware` (registered in `AppModule.configure()`, same place `RequestIdMiddleware` already runs), which executes before Guards and therefore records everything.
2. Reading `response.statusCode` synchronously inside an interceptor's `tap({ next, error })` would also have mislabeled every error response — `GlobalExceptionFilter` (which actually sets the real status code) runs *outside and after* the interceptor pipeline, so that status wouldn't be final yet at the point `tap`'s error callback fires. Fixed by reading `response.statusCode` from Express's own `finish` event instead, which only fires once the response has actually been sent, guaranteeing the status is final regardless of which path produced it.

**Decision (response envelope) — a `GET /metrics` bug caught live:** `ResponseInterceptor` unconditionally wraps every successful response in `{success, data, requestId}`. The first version of `GET /metrics` came back as that envelope with Prometheus's plain-text exposition format escaped inside a JSON string — completely unscrapeable. Added a `@RawResponse()` decorator (mirroring `@Public()`'s pattern) that `ResponseInterceptor` checks via `Reflector` and skips wrapping for; applied to `MetricsController` only. Verified live: `GET /metrics` now returns real `text/plain` Prometheus output, while every other endpoint's envelope is unchanged (spot-checked `GET /health` after the change).

**Verified live, throughout:** every fix above was confirmed against the real running API, not just by code review — the Redis storage fix via inspecting the actual Redis key/TTL/increment behavior and a real `429` after exceeding the limit; the metrics middleware fix via a real unauthenticated request appearing correctly labeled in `GET /metrics` output; the response-envelope fix via `GET /metrics` returning parseable Prometheus text. Full workspace build/typecheck/lint/test stayed green throughout (see the Phase 12 commit for final counts).
